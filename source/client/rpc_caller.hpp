...
public:
  ...
  bool call(const BaseConnection::ptr &, const std::string &,
            const Json::Value &, Json::Value &); // 同步
  bool call(const BaseConnection::ptr &, const std::string &,
            const Json::Value &, JsonAsyncResponse &); // 异步
  bool call(const BaseConnection::ptr &, const std::string &,
            const Json::Value &, const JsonResponseCallback &); // 回调

private:
  Requestor::ptr _requestor;

  void Callback(std::shared_ptr<std::promise<Json::Value>>,
                const BaseMessage::ptr &);
  void CallerCallback(const JsonResponseCallback &,
                      const BaseMessage::ptr &); // 用于回调请求的回调
};

RpcCaller::RpcCaller(const Requestor::ptr &requestor) : _requestor(requestor) {}

bool RpcCaller::call(const BaseConnection::ptr &con, const std::string &method,
  ...
  */
  ...
  // 发送请求
  if (!_requestor->send(con, std::dynamic_pointer_cast<BaseMessage>(req),
                        rsp)) {
    ELOG("Rpc同步请求失败\n");
    return false;
  }
...
bool RpcCaller::call(const BaseConnection::ptr &con, const std::string &method,
  ...
  result =
      json_promise
          ->get_future(); // 获取相关联的future对象
                          // 上层最终将得到这样的future对象从而再次对future进行处理从而得到最终结果
...
bool RpcCaller::call(const BaseConnection::ptr &con, const std::string &method,
  ...
  Requestor::RequestCallback req_cb =
      std::bind(&RpcCaller::CallerCallback, this, cb,
                std::placeholders::_1); // bind 成了一个 void(const
                                        // Rpc::BaseMessage::ptr &)类型的函数
...
void RpcCaller::CallerCallback(const JsonResponseCallback &cb,
...
void RpcCaller::Callback(std::shared_ptr<std::promise<Json::Value>> result,
  ...
  result->set_value(
      rpc_rsp
          ->result()); // 主要通过异步 这里让上层以回调的方式处理result
                       // 当通过 set_value 设置参数时即表示调用
                       // 调用完毕后结果将进行存储在相关联的future对象中
                       // 用户可以直接调用future对象中的get()函数来直接获取结果

  /*
      这里本质上就是用户在调用call时将传递对应的参数
     参数中用户需要传入一个future对象的引用以便于对应promise对象的相关联future对象的获取
